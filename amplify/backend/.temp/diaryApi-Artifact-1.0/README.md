...
Copy diaryApi-Artifact-1.0.jar from the `target` folder and all other libraries located in `target/lib` to your project's lib folder.
...
// create a client
final DiaryApiClient client = factory.build(DiaryApiClient.class);

// Invoke your diaryOptions method
 output = client.diaryOptions();
